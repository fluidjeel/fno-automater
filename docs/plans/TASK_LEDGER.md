...
Use statuses `READY`, `IN_PROGRESS`, `BLOCKED`, `DONE`. Next READY: ADESK-A7 (ADESK-A6 DONE; bias battery 11 metrics).
...
| ADESK-A7 | READY | `ImprovementRecord` + clustering | Table + dedupe; `trading evaluate improvements` | ADESK-A1 |
...
Next: ADESK-A7 ImprovementRecord; Monday Fyers + CAS depth + supervised paper; PAPER-005 after live paper evidence.
```
