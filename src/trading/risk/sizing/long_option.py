"""Long call/put sizing: premium at risk plus charges and slippage buffer."""
...
__all__ = ["LongOptionSizingEngine", "LongOptionSizingResult", "LotBounds"]
...
class LotBounds:
    """Individual terms in the min() sizing formula."""
    ...
    def binding_constraint(self, approved_lots: int) -> SizingBindingConstraint:
        for constraint in _CONSTRAINT_ORDER:
            ...
            if value == approved_lots:
                return constraint
        return SizingBindingConstraint.RISK


@dataclass(frozen=True, slots=True)
class LongOptionSizingResult:
    """Sized outcome before capital reservation."""
...
class LongOptionSizingEngine:
    """Size premium-paid single-leg long options."""
    ...
    ) -> LongOptionSizingResult:
        """Compute lots from the min() of risk, capital, margin and limit terms."""
        intent = request.intent
        if len(intent.legs) != 1:
            raise ValueError("long option sizing supports exactly one leg")
        leg = intent.legs[0]
        if leg.side is not Side.BUY:
            raise ValueError("long option sizing requires a BUY leg")
        if instrument.instrument_kind is not InstrumentKind.OPTION:
            raise ValueError("long option sizing requires an OPTION instrument")
    ...
    def _entry_price(feature: FeatureSnapshot) -> Price:
        ...
        if ask is None:
            raise ValueError("ask price is required to size a long option entry")
        return ask
